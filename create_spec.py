    ...
    for track_id in tracks.index:
        try:
            ...
            for i in range(int(x.size / sr / 3.)):
                out = spec_path + tracks['set', 'split'].loc[track_id] + '/' + tracks['track', 'genre_top'].loc[track_id] + '/spectrogram_' + str(track_id) + '_' + str(i) + '.png'
                if tracks['track', 'genre_top'].loc[track_id] == 'Old-Time / Historic':
                    out = spec_path + tracks['set', 'split'].loc[track_id] + '/' + 'Old-Time - Historic' + '/spectrogram_' + str(track_id) + '_' + str(i) + '.png'
                lower = 3*i*sr
                if lower + 3*sr > x.size:
                    upper = x.size
                else:
                    upper = lower + 3*sr
                spectrogram_image(x[3*i*sr:upper], sr=sr, out=out, hop_length=512, n_mels=128)
        except:
            continue
...
def create_spec_gtzan():
    ...
    for genre in os.listdir(path):
        ...
        for file in os.listdir(path+genre+"/"):
            x, sr = librosa.load(path+genre+"/"+file, offset=0.0, sr=None, mono=True)
            for i in range(int(x.size/sr/3.)):
                lower = 3*i*sr
                if lower + 3*sr > x.size:
                    upper = x.size
                    ...
                    upper = lower + 3*sr
                if (j % 10 == 0):
                    out = "D:/gtzan_spec/test/" + genre + "/" + str(file) + str(i) + '.png'
                else:
                    out = "D:/gtzan_spec/train/" + genre + "/" + str(file) + str(i) + '.png'
                spectrogram_image(x[3*i*sr:upper], sr=sr, out=out, hop_length=512, n_mels=128)
            j+=1
