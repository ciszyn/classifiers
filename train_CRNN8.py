...
def train():
    model_path = './models/'+set_size+'/fma.crnn8'
    ...
    try:
        ...
        model.add(layers.MaxPooling2D((2, 2)))
        model.add(layers.Dense(64, activation='relu'))
        model.add(layers.Reshape((105, 64)))
    ...
    try:
        dump(history.history, "./history/" + set_size + '/' + 'crnn8.joblib')
    except:
        os.makedirs("./history/" + set_size)
        dump(history.history, "./history/" + set_size + '/' + 'crnn8.joblib')
    model.save('./models/'+set_size+'/fma.crnn8')
