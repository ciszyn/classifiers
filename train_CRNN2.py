...
from modelScripts.CRNN2 import getModel
...
def train():
    model_path = './models/'+set_size+'/fma.crnn2'
    ...
    try:
        dump(history.history, "./history/" + set_size + '/' + 'crnn2.joblib')
    except:
        os.makedirs("./history/" + set_size)
        dump(history.history, "./history/" + set_size + '/' + 'crnn2.joblib')
    model.save('./models/'+set_size+'/fma.crnn2')
