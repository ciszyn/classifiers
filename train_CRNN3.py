...
from modelScripts.CRNN3 import getModel
...
def train():
    model_path = './models/'+set_size+'/fma.crnn3'
    ...
    try:
        dump(history.history, "./history/" + set_size + '/' + 'crnn3.joblib')
    except:
        os.makedirs("./history/" + set_size)
        dump(history.history, "./history/" + set_size + '/' + 'crnn3.joblib')
    model.save('./models/'+set_size+'/fma.crnn3')
