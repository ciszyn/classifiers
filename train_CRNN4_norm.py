...
def train():
    model_path = './models/'+set_size+'/fma.crnn4n'
    ...
    try:
        ...
        model = Sequential()
        model.add(layers.Conv2D(32, (5, 5), activation='relu', input_shape=(128, 259, 1)))
    ...
    history = model.fit(train_it, epochs=5, validation_data=val_it, callbacks=[callback])
    try:
        dump(history.history, "./history/" + set_size + '/' + 'crnn4n.joblib')
    except:
        os.makedirs("./history/" + set_size)
        dump(history.history, "./history/" + set_size + '/' + 'crnn4n.joblib')
    model.save('./models/'+set_size+'/fma.crnn4n')
