...
def create_spectrograms_full():
    for track_id in tracks.index:
        try:
            ...
            out = spec_full_path + tracks['set', 'split'].loc[track_id] + '/' + tracks['track', 'genre_top'].loc[track_id] + '/spectrogram_' + str(track_id) + '_full' + '.png'
            if tracks['track', 'genre_top'].loc[track_id] == 'Old-Time / Historic':
                out = spec_full_path + tracks['set', 'split'].loc[track_id] + '/' + 'Old-Time - Historic' + '/spectrogram_' + str(track_id) + '_full' + '.png'
