...
import scipy
from voting import normalize, prepare_data
...
def evaluate_weights_flexible(weights, yhats, y, n):
    ...
    summed = tensordot(yhats, [1 for _ in range(len(weights))], axes=((0),(0)))
...
def main(method, n):
    ...
    try:
        ...
        outputs, real_outputs = prepare_data(keras_models, sklearn_models, val, 'validation')
        dump(outputs, './votingVars/predictions_val.joblib')
        dump(real_outputs, './votingVars/labels_val.joblib')
        outputs_test, real_outputs_test = prepare_data(keras_models, sklearn_models, test, 'test')
    ...
    for combination, index_combination in zip(combinations, index_combinations):
        ...
        testLoss = evaluate_weights_flexible(weights, [outputs_test[i] for i in index_combination], real_outputs_test, n)
    ...
    for oldLoss, index_combination, combination, oldTestLoss in performances[:50]:
        ...
        for k in range(count):
            if method == 'differential_evolution':
                weights_j = differential_evolution(evaluate_weights_flexible, bounds=bound_w, args=(combination, real_outputs, n), maxiter=1000, tol=1e-7)['x']
            else:
                weights_j = minimize(evaluate_weights_flexible, args=(combination, real_outputs, n), bounds=bound_w, x0=[1 for _ in range(len(combination) * n)], method=method)['x']
            loss_j = evaluate_weights_flexible(weights_j, combination, real_outputs, n)
            test_acc_j = 1-evaluate_weights_flexible(weights_j, [outputs_test[i] for i in index_combination], real_outputs_test, n)
            test_acc += test_acc_j
            loss += loss_j
        weights = weights_j
