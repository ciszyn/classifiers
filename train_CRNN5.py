...
def train():
    model_path = './models/'+set_size+'/fma.crnn5'
    ...
    try:
        ...
        model.add(layers.MaxPooling2D((2, 2)))
        model.add(layers.Reshape((30*63, 32)))
        ...
        model.add(layers.Dense(64, activation='relu')) # mniej neuronów, 32/16...
    ...
    history = model.fit(train_it, epochs=10, validation_data=val_it, callbacks=[callback])
    try:
        dump(history.history, "./history/" + set_size + '/' + 'crnn5.joblib')
    except:
        os.makedirs("./history/" + set_size)
        dump(history.history, "./history/" + set_size + '/' + 'crnn5.joblib')
    model.save('./models/'+set_size+'/fma.crnn5')
