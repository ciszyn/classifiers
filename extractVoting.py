...
def votingSummary():
    ...
    for optimizationMethod in optimizationMethods:
        for votingMethod in votingMethods:
            try:
                df = pd.read_csv('./votingVars/' + votingMethod +
                                 '_' + optimizationMethod + '.csv')
                resultDf.loc[optimizationMethod + '_' +
                             votingMethod, 'mean'] = df.loc[0, 'mean']
                resultDf.loc[optimizationMethod + '_' +
                             votingMethod, 'variance'] = df.loc[0, 'variance']
                resultDf.loc[optimizationMethod + '_' +
                             votingMethod, 'p_value'] = df.loc[0, 'p_value']
                resultDf.loc[optimizationMethod + '_' + votingMethod,
                             'max_accuracy'] = df.loc[0, 'max_accuracy']
                resultDf.loc[optimizationMethod + '_' + votingMethod,
                             'max_increment'] = df.loc[0, 'max_increment']
            except:
                continue
    ...
    for optimizationMethod in optimizationMethods:
        for votingMethod in votingMethods:
            try:
                ...
                for key in dict:
                    df.loc[len(df)] = [key, dict[key]["val_acc_1"], dict[key]
                                       ["val_acc_2"], dict[key]["test_acc_1"], dict[key]["test_acc_2"]]
                df["differences"] = df["test_acc_2"] - df["test_acc_1"]
                count = df.at[0, "count"] = df['differences'].size
                mean = df.at[0, "mean"] = statistics.mean(
                    df["differences"].to_numpy())
                variance = df.at[0, "variance"] = statistics.stdev(
                    df["differences"].to_numpy())
